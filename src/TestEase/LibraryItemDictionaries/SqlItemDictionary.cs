...
namespace TestEase.LibraryItemDictionaries
{
    ...
    public class SqlItemDictionary : BaseItemDictionary
    {
        ...
        {
            if (GetQueuedSql.Count == 0) throw new ArgumentException("No sql is currently queued");
            ...
            foreach (var queuedSqlKey in GetQueuedSql.Keys)
            {
                if (!connections.ContainsKey(queuedSqlKey.ToUpper()))
                    throw new KeyNotFoundException(
                        $"Connections does not contain a key for {queuedSqlKey.ToUpper()}. Keys present: {string.Join(", ", connections.Keys.ToList())}");
        ...
        ///     The file cannot be found, such as when
        ///     <see cref="System.IO.FileMode" /> is FileMode.Truncate or FileMode.Open, and the file specified by path does not
        ...
        ///     The requested is not permitted by the operating system for the specified
        ///     path, such as when <see cref="System.IO.FileAccess" /> is Write or ReadWrite and the file or directory is set for
        ///     read-only access.
        /// </exception>
        ...
        public SqlItemDictionary QueueLibraryItem(string scriptKey,
            IDictionary<string, object> replacementValues = null)
        {
            ...
            if (scripts.Length <= 1)
                throw new ArgumentException(
                    $"DB type split did not work correctly. Expression used: --DbType \\\\s*=\\\\s* ({scriptSplitter})");
        ...
        public void SetupConnections(IDictionary<string, string> connectionsToAdd)
        {
            ...
            foreach (var key in connectionsToAdd.Keys) connections.Add(key.ToUpper(), connectionsToAdd[key]);
        }
